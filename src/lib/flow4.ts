...
async function ensureSchema(): Promise<void> {
  ...
  try {
    await exec(`ALTER TABLE course_chapters ADD COLUMN IF NOT EXISTS course_slug VARCHAR(191) NULL`);
  } catch {}
  try {
    await exec(`ALTER TABLE course_chapters ADD COLUMN IF NOT EXISTS content_type VARCHAR(24) NOT NULL DEFAULT 'class'`);
  } catch {}
  ...
  try {
    await exec(`ALTER TABLE course_subject_assignments ADD COLUMN IF NOT EXISTS sort_order INT NOT NULL DEFAULT 0`);
  } catch {}
