...
async function ensureTables(): Promise<void> {
  ...
  try {
    await exec(`ALTER TABLE exams ADD COLUMN IF NOT EXISTS type ENUM('public','course') NOT NULL DEFAULT 'public' AFTER kind`);
  } catch {
    // Best effort — column may already exist.
  }
  try {
  ...
  try {
    await exec(`ALTER TABLE exams ADD COLUMN IF NOT EXISTS active TINYINT(1) NOT NULL DEFAULT 1 AFTER featured`);
  } catch {
    // Best effort — column may already exist.
  }
  try {
    await exec(`ALTER TABLE exams ADD COLUMN IF NOT EXISTS attempt_limit INT NOT NULL DEFAULT 0 AFTER second_timer_deduction`);
  } catch {
    // Best effort — column may already exist.
  }
  ...
  try {
    await exec(`ALTER TABLE exam_results ADD COLUMN IF NOT EXISTS snapshot_marks DECIMAL(6,2) NULL`);
    await exec(`ALTER TABLE exam_results ADD COLUMN IF NOT EXISTS snapshot_negative_per_wrong DECIMAL(4,2) NULL`);
    await exec(`ALTER TABLE exam_results ADD COLUMN IF NOT EXISTS snapshot_second_timer_deduction DECIMAL(6,2) NULL`);
    await exec(`ALTER TABLE exam_results ADD COLUMN IF NOT EXISTS snapshot_duration_minutes INT NULL`);
    await exec(`ALTER TABLE exam_results ADD COLUMN IF NOT EXISTS snapshot_negative_enabled TINYINT(1) NULL`);
    await exec(`ALTER TABLE exam_results ADD COLUMN IF NOT EXISTS snapshot_second_timer_enabled TINYINT(1) NULL`);
  } catch {
    // Best effort — columns may already exist.
  }
}
