...
export default function SubjectContentsPage({
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      ...
      <h1 className="mt-3 text-xl font-extrabold text-heading">Subject Contents</h1>
      <p className="mt-1 text-xs text-neutral-500">
        <span className="font-bold text-primary-500">Course Content → Subject → Content</span> · {subjectName || subjectId} — Manage contents directly under this subject (video, PDF, notes, image, audio, quiz, etc.)
      </p>
      <p className="mt-1 text-xs text-neutral-400">Supports all existing content types: video, PDF, notes, image, audio, quiz, etc.</p>
      ...
      <div className="mt-6 rounded-2xl border border-ink/10 bg-white admin-dark:border-[#1e3a65] admin-dark:bg-[#112544] p-4 sm:p-5">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <div className="sm:col-span-2">
            ...
            <input placeholder="e.g. Biology Class 1, Chemistry PDF, Physics Quiz" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="mt-1 w-full rounded-xl border border-primary-500/40 bg-[#f8fbff] admin-dark:bg-[#0f2547] px-3.5 py-2.5 text-sm text-heading outline-none" />
          </div>
          <div>
      ...
      <div className="mt-6">
        ...
        {contents.length === 0 ? (
          <p className="mt-3 rounded-xl border border-dashed border-ink/15 px-4 py-8 text-center text-sm text-neutral-500">No content yet. Add Class 1, Notes etc. above. Students will see &quot;No Content Available&quot; until you publish.</p>
        ) : (
          <ul className="mt-3 space-y-2">
            {contents.map((c, idx) => (
              <li key={c.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-[#dbeafe] bg-white admin-dark:border-[#1e3a65] admin-dark:bg-[#112544] p-4">
                ...
                <button type="button" disabled={busy} onClick={async () => { if (window.confirm(`Delete "${c.title}"?`)) await post({ action: "delete-direct-content", id: c.id }, "Removed."); }} className="rounded-lg border border-red-500/30 bg-red-500/5 px-3 py-1.5 text-xs font-bold text-red-400">Delete</button>
