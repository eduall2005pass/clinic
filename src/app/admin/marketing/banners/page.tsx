...
export default function PromotionalBannersPage() {
  ...
  return (
    <section className="mx-auto max-w-3xl px-4 py-8 sm:px-6 sm:py-10">
      ...
      {/* List */}
      {!banners && (
        <p className={`${cardClass} mt-6 py-6 text-center text-sm font-semibold text-zinc-500`}>
          Loading…
        </p>
      )}
      {banners?.length === 0 && (
        <p className={`${cardClass} mt-6 py-8 text-center text-sm font-semibold text-zinc-500`}>
          No banners yet.
        </p>
      )}
        ...
        {banners?.map((banner, index) => (
          <li key={banner.id} className={`${cardClass} p-4`}>
            <div className="flex flex-wrap items-center gap-3">
              <span className="relative h-12 w-24 shrink-0 overflow-hidden rounded-lg border border-neutral-200 bg-neutral-100 admin-dark:border-zinc-700 admin-dark:bg-zinc-800">
                <Image
                  src={banner.url}
            ...
            <div className="mt-3 grid gap-3">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button
                  ...
                  className="ml-auto self-start rounded-xl border border-neutral-200 px-4 py-2 text-xs font-bold text-zinc-600 transition hover:border-primary-500/60 hover:text-primary-600 admin-dark:border-zinc-700 admin-dark:text-zinc-300"
                >
                  Save Changes
                </button>
              </div>
